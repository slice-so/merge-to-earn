...
const proposeTransaction = async (mints: Mint[]) => {
  ...
  try {
    ...
    const { accounts, tokenDiffs } = await updateAccountsToMint(nonce, mints)
    const resliceData = formatResliceData(accounts, tokenDiffs)

    const { safeTxGas } = await getEstimate(resliceData)
...
const updateAccountsToMint = async (nonce: number, mints: Mint[]) => {
...
const formatResliceData = (accounts: string[], tokenDiffs: number[]) =>
  ethers.utils
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  const status = await proposeTransaction(accountsToReslice)
